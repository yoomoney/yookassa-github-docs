...
Полученный код вставьте в html-документ или добавьте на страницу вашего сайта.

Примечание: обязательными параметрами платежа являются **shopId**, **scid**, **customerNumber** и **sum**, без них платеж работать не будет. Остальные параметры на ваше усмотрение ([документация](https://tech.yandex.ru/money/doc/payment-solution/payment-form/payment-form-http-docpage/) по параметрам формы). Обратите внимание, что для демо и боевых платежей, значение shopId одинаковое; значение scid для демо будет одно, а для боя другое (боевой scid вы получите от нас после того, как мы напишите нам о том, что демо-платежи проходят успешно и мы включим для вашего shopId возможность платить в реальном режиме).
