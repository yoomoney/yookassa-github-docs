...
На этой странице вы найдете описание интеграции, примеры и пошаговый план тестирования:

 [#](https://github.com/yandex-money/yandex-money-joinup/blob/master/demo/пример%20платежной%20формы.html) html-код минимальной платежной формы  
 [#](#Описание-процесса-платежа) объяснение процесса платежа
 [#](#Шаг-2-Скрипты-checkurl-и-avisourl-колбеки) указания по скриптам взаимодействия  
 [#](#21-Проверка-md5-суммы-при-ответах-на-запросы-checkorder-и-paymentaviso-документация) как защитить сумму платежа от подмены  
* а также:
  * решение ошибок при тестировании
  * пример запроса на CURL (с помощью которого можно выполнить отладку ответов ваших скриптов)
* рекомендации по использованию кнопки "Заплати через Яндекс" повышающую удобство платежа и конверсию.

##### Все ссылки
* [Пример платежной формы](https://github.com/yandex-money/yandex-money-joinup/blob/master/demo/пример%20платежной%20формы.html) _(минимальный html-код, достачный для проведения оплаты)_;
...
* Вы должны зарегистрироваться в Яндекс.Кассе и заполнить нашу техническую анкету;
* Получить от нас идентификаторы **shopId** и **scid**;
* Ваш сайт должен работать по HTTPS протоколу;
* Ваши скрипты checkURL и avisoURL должны [корректно](#Примеры-ответов-колбеков) отвечать на наши запросы
...
##### Лимиты суммы оплаты
У каждого платежного метода есть свои лимиты (минимальный и максимальный размер суммы). Лимиты одинаковы как в демо среде, так и в боевой (учитывайте это во время тестирования). См. [таблица лимитов платежных методов](https://money.yandex.ru/doc.xml?id=527483&ncrnd=6790). По-умолчанию, вы можете не предусматривать функционал проверки суммы, при оплате, т.к. в случае, если сумма не будет проходить по лимитам, мы сообщим об этом плательщику на странице оплаты. В демо-среде рекомендуем тестировать не большими суммами, например, 100 руб. (sum="100.00").

##### Тестовый демо-кошелек
1. **Создайте свой тестовый кошелек** за 1 минуту: пройдите по ссылке https://demomoney.yandex.ru/reg/ (вы сможете сделать себе тестовый кошелек, если у вас уже есть учетная запись в яндекс; если ее нет, ее надо создать, чтобы затем получить персональный тестовый кошелек; обратите внимание, что в реальном режиме кошелек должен иметь [минимальную идентификацию](https://money.yandex.ru/security/identification/) уровня "Анонимный").
