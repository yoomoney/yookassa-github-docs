...
Файл "пример платежной формы.html" содержит необходимый код, чтобы выполнить тестовый платеж. Скопируйте к себе и отредактируйте код, прописав в него ваши значения **shopId** и **scid**. В файле примера это будут следующие строки:
...
Полученный код вставьте в html-документ или добавьте на страницу вашего сайта. Обязательными параметрами платежа являются **shopId**, **scid**, **customerNumber** и **sum**, без них платеж работать не будет. Остальные параметры на ваше усмотрение ([документация](https://tech.yandex.ru/money/doc/payment-solution/payment-form/payment-form-http-docpage/) по параметрам формы). Обратите внимание, что демо и боя, значение  shopId одинаковое; значение scid для демо будет одно, а для боя другое (боевой scid вы получите от нас после того, как проведете успешный демо-платеж).
