...
# Обязательно замените старый хост payment.yandex.net на новый api.yookassa.ru
...
Если для взаимодействия вы используете устаревшую библиотеку [SDK](https://yookassa.ru/developers/using-api/using-sdks), перейдите на новые. Новые библиотеки SDK не используют старый адрес, они используют новый. 
...
+ new API endpoint: https://api.yookassa.ru/v3/
```
Смена хоста - это единственное изменение, которое требуется, чтобы вы внесли в код своей системы, чтобы в определенный момент платежи не сломались.
