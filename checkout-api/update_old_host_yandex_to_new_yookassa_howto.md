...
* Если отправить запрос на создание платежа платежным методом `"yandex_money"` (старое название) на https://payment.yandex.net/api/v3/, вы получите ошибку, что такого метода не существует. Поэтому на новый хост нужно делать запрос только с новым названием метода -- "yoo_money".
