...
* На `return_url` мы не шлем никаких платежных данных (некоторые разработчики имеют предположение, что было бы удобней получать платежные данные именно на `return_url`). Для получения данных о платеже служит именно [URL для уведомлений](/checkout-api/031-01%20url%20для%20уведомлений.md).
